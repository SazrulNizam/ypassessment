<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Assign</h2>
    </x-slot>
...
<div class="py-12 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 border-b pb-4 flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900">Manage Subject</h1>
                <p class="mt-2 text-sm text-gray-600">Classroom: <span class="font-bold text-blue-600">{{ $assigned->name }}</span></p>
            </div>
            <a href="{{ route('class.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Back</a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        @forelse($availableSubject as $subject)
                            <div class="group flex justify-between items-center p-4 bg-gray-50 rounded-xl hover:bg-yellow-50 transition-all duration-200 border border-transparent hover:border-yellow-200">
                                ...
                                <form action="{{ route('post-subject.index', $assigned->id)}}" method="POST">
                                    @csrf
                        ...
                        @empty
                            <div class="text-center py-8">
                                <p class="text-gray-400 italic">Tiada subjek tersedia untuk ditambah.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        @forelse($assigned->subject as $subject) {{-- Guna 'user' ikut nama function model anda --}}
                            <div class="flex justify-between items-center p-4 bg-blue-50 rounded-xl border border-blue-100">
                                <div class="flex items-center">
                                    ...
                                    <span class="font-medium text-blue-900">{{ $subject->name }}</span>
                                </div>
                                <form action="{{ route('remove-subject.index', $assigned->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="subject_id" value="{{ $subject->id }}">
                        ...
                        @empty
                            <div class="text-center py-8">
                                <p class="text-gray-400 italic">Belum ada subject didaftarkan dalam kelas ini.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
